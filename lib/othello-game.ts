...
export class OthelloGame {
  ...
  private getFlippedPieces(row: number, col: number, player: Player): Position[] {
    const directions = [
      [-1, -1],
      [-1, 0],
      [-1, 1],
    ...
    for (const [dr, dc] of directions) {
      ...
      if (r >= 0 && r < 8 && c >= 0 && c < 8 && this.board[r][c] === player && piecesToFlip.length > 0) {
  ...
  private getMinimaxMove(depth: number): Position | null {
    ...
    for (const move of validMoves) {
      ...
      const score = this.minimax(gameCopy, depth - 1, Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY, false)
  ...
  private minimax(game: OthelloGame, depth: number, alpha: number, beta: number, isMaximizing: boolean): number {
