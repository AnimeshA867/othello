 ...
 */
async function findOrCreateRoom(userId, playerName, userRank, rankSetType) {
    ...
    availableRooms.sort((a, b) => {
        var _a, _b;
        const rankA = ((_a = a.players[0]) === null || _a === void 0 ? void 0 : _a.rank) || 0;
        const rankB = ((_b = b.players[0]) === null || _b === void 0 ? void 0 : _b.rank) || 0;
    ...
    if (availableRooms.length > 0) {
        ...
        // Create new room
        const roomId = (0, gameLogic_1.generateRoomId)();
        const player = {
            id: userId,
            name: playerName,
            rank: userRank,
        ...
        const room = {
            ...
            rankSetType,
            status: "waiting",
            gameState: (0, gameLogic_1.createInitialGameState)(),
            createdAt: new Date(),
        };
 ...
 */
function createRoom(userId, playerName, userRank = 1000, rankSetType = "beginner") {
    const roomId = (0, gameLogic_1.generateRoomId)();
    console.log(`Creating new room with ID ${roomId} for player ${playerName || userId}`);
    const player = {
        id: userId,
        name: playerName,
        rank: userRank,
    ...
    rooms.set(roomId, room);
    playerToRoom.set(userId, roomId);
    console.log(`New room ${roomId} created by ${playerName || userId}. Total rooms: ${rooms.size}`);
    return room;
}
/**
 * Join an existing room by ID
 */
function joinRoom(roomId, userId, playerName, userRank = 1000) {
    console.log(`Attempting to join room ${roomId} with player ${playerName || userId}`);
    const room = rooms.get(roomId);
    if (!room) {
        console.error(`Room ${roomId} not found. Available rooms: ${Array.from(rooms.keys()).join(", ")}`);
        throw new Error(`Room ${roomId} not found`);
    }
    if (room.status !== "waiting") {
        console.error(`Room ${roomId} is not in waiting status (current: ${room.status})`);
        throw new Error(`Room ${roomId} is not in waiting status`);
    }
    if (room.players.length >= 2) {
        console.error(`Room ${roomId} is already full (${room.players.length} players)`);
 ...
 */
function updateGameState(roomId, userId, gameState) {
    const room = rooms.get(roomId);
    if (!room) {
        throw new Error(`Room ${roomId} not found`);
    }
