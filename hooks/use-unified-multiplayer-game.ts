...
interface UnifiedMultiplayerGameState extends GameState {
  localPlayer: Player | null;
  opponentName?: string;
  opponentRank?: number;
  isWaitingForPlayer: boolean;
...
export function useUnifiedMultiplayerGame() {
  ...
  const [gameState, setGameState] = useState<UnifiedMultiplayerGameState>({
    board: Array(8)
      .fill(null)
      ...
      switch (message.type) {
        ...
        case "game_ready":
          setGameState((prev) => {
            ...
            const localRole = prev.localPlayer ?? websocketState.playerRole;
...
export function useRankedMultiplayerGame() {
  const {
    websocketState,
    sendMessage,
    createRoom,
  ...
  const [gameState, setGameState] = useState<RankedMultiplayerGameState>({
    board: Array(8)
      .fill(null)
      .map(() => Array(8).fill(null)),
    currentPlayer: "black",
      ...
      switch (message.type) {
        ...
        case "game_state":
          ...
          setGameState((prev) => {
            ...
            // Keep the original validMoves from the server
            const newState: RankedMultiplayerGameState = {
              ...prev,
              ...message.gameState,
              board: updatedBoard,
              localPlayer: prev.localPlayer,
              ...
              isWaitingForPlayer: prev.isWaitingForPlayer,
              opponentName: prev.opponentName,
              opponentRank: prev.opponentRank,
              rankSetType: prev.rankSetType,
            };
  ...
  const makeMove = useCallback(
    (row: number, col: number) => {
      console.log("Attempting ranked move at", row, col);
      console.log("Current ranked state:", {
